...
/**
 * All getters/setters on this class are generated by Lombok's {@code @Getter}/{@code @Setter}
 * (see below) rather than written out by hand.
 * <p>
 * If your IDE reports "cannot find symbol" for methods like {@code getStartPiPerSecond()} even
 * though {@code mvn compile} succeeds, your IDE is missing Lombok annotation processing support,
 * not the codebase. Install the Lombok plugin for your IDE and enable annotation processing —
 * see <a href="https://projectlombok.org/setup/">projectlombok.org/setup</a> for IDE-specific
 * instructions (IntelliJ, Eclipse, VS Code).
 */
@Configuration
