...
/**
 * Integration test for the {@code backpressure} strategy (classic rate adjustment).
 * Verifies that the classic scheduler and rate calculator are active,
 * and no Bucket4j beans are created.
 */
@SpringBootTest(properties = {
    ...
    "benchmark.bpmnProcessId=backpressure-strategy-test"
})
@CamundaSpringProcessTest
class BackpressureStrategyIT {
    ...
    private static final String TRIVIAL_BPMN = """
            ...
            <bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL"
                              ...
                              id="Definitions_1">
              <bpmn:process id="backpressure-strategy-test" name="Backpressure Strategy Test" isExecutable="true">
    ...
    void classicSchedulerIsActive() {
        assertNotNull(startPiScheduler,
                "StartPiScheduler should be active for 'backpressure' strategy");
    }

    @Test
    void bucket4jBeansAreNotCreated() {
        assertFalse(context.containsBean("bucket4jPiScheduler"),
                "Bucket4jPiScheduler should NOT be active for 'backpressure' strategy");
        assertFalse(context.containsBean("flowControlInterceptor"),
                "FlowControlInterceptor should NOT be active for 'backpressure' strategy");
        assertFalse(context.containsBean("flowControlBucket"),
                "flowControlBucket should NOT be active for 'backpressure' strategy");
    }

    @Test
    void rateAdjustmentDoesNotThrow() {
        // The adjustStartRates method should handle 'backpressure' strategy
        // without errors, even with zero traffic
        assertDoesNotThrow(() -> startPiScheduler.adjustStartRates(),
                "Rate adjustment should not throw for 'backpressure' strategy");
    }
    ...
    void deployAndStartProcessInstance() {
        client.newDeployResourceCommand()
                .addResourceStream(
                        ...
                        "backpressure-strategy-test.bpmn")
                .send()
                .join();

        var result = client.newCreateInstanceCommand()
                .bpmnProcessId("backpressure-strategy-test")
                .latestVersion()
                .send()
                .join();

        assertTrue(result.getProcessInstanceKey() > 0,
                "Should successfully start a process instance with 'backpressure' strategy");
    }
}
